...
sub EnsureProperATPProduction {
	my $self = shift;
	my $args = Bio::KBase::ObjectAPI::utilities::args(["fbamodel"],{
		anaerobe => 0,
		max_objective_limit => 1.4
	}, @_);
	#Retrieving and hashing core reactions
	my $corerxns = Bio::KBase::constants::core_reactions();
	my $noncorehash = {};
	my $rxnhash = {};
	for (my $i=0; $i < @{$corerxns}; $i++) {
		$rxnhash->{$corerxns->[$i]->[0]} = $corerxns->[$i]->[1];
	}
	#Creating empty core model linked to core template
	my $template_trans = Bio::KBase::constants::template_trans();
	my $coremdl = Bio::KBase::ObjectAPI::KBaseFBA::FBAModel->new({
		id => "core",
		source => Bio::KBase::utilities::conf("ModelSEED","source"),
		source_id => "core",
		type => "core",
		name => "core",
		genome_ref => $args->{fbamodel}->genome_ref(),
		template_ref => Bio::KBase::utilities::conf("ModelSEED","default_template_workspace")."/".$template_trans->{core},
		template_refs => [Bio::KBase::utilities::conf("ModelSEED","default_template_workspace")."/".$template_trans->{core}],
		gapfillings => [],
		gapgens => [],
		biomasses => [],
		modelcompartments => [],
	...
	$coremdl->parent($self->parent());
	#Removing and blacklisting aerobic reactions if model is anaerobic
	my $blacklist = [];
	if ($args->{anaerobe} == 1) {
		my $mdlrxn = $args->{fbamodel}->getObject("modelreactions","rxn14419_c0");
		if (defined($mdlrxn)) {
			$args->{fbamodel}->remove("modelreactions",$mdlrxn);
		}
		$mdlrxn = $args->{fbamodel}->getObject("modelreactions","rxn14422_c0");
		if (defined($mdlrxn)) {
			$args->{fbamodel}->remove("modelreactions",$mdlrxn);
		}
		$blacklist = ["rxn14426","rxn14419","rxn14422"];
	}
	#Adding core reactions from model to core model
	my $mdlrxns = $args->{fbamodel}->modelreactions();
	for (my $i=0; $i < @{$mdlrxns}; $i++) {
		if ($mdlrxns->[$i]->id() =~ m/(rxn\d+)/ && defined($rxnhash->{$1})) {
			#Making sure reaction has core template directionality
			$mdlrxns->[$i]->direction($rxnhash->{$1});
			$coremdl->merge_in_reaction($mdlrxns->[$i]);
		} else {
			$noncorehash->{$mdlrxns->[$i]->id()} = 1;
		}
	}	
	#Gapfilling core model to ensure ATP production
	$coremdl->add("biomasses",Bio::KBase::constants::atp_hydrolysis_biomass());
	my $output = Bio::KBase::ObjectAPI::functions::func_gapfill_metabolic_model({
		workspace => "NULL",
		fbamodel_id => "core",
		fbamodel_output_id => "core.gf",
		target_reaction => "bio2",
		media_workspace => Bio::KBase::utilities::conf("ModelSEED","default_media_workspace"),
		media_id => "RefGlucoseMinimal",
		atp_production_check => 0,
	...
	#Adding gapfilled reactions to original model
	$mdlrxns = $coremdl->modelreactions();
	for (my $i=0; $i < @{$mdlrxns}; $i++) {
		if ($mdlrxns->[$i]->gapfillString() ne "") {
			my $mdlrxn = $args->{fbamodel}->getObject("modelreactions",$mdlrxns->[$i]->id());
			if (!defined($mdlrxn)) {
				$args->{fbamodel}->merge_in_reaction($mdlrxns->[$i]);
			} else {
				delete($noncorehash->{$mdlrxns->[$i]->id()});
				$mdlrxn->direction($mdlrxns->[$i]->direction());
			}
			print "Gapfilled:".$mdlrxns->[$i]->id()."\n";
		}
	}
	#Adding all other reactions in ATP-safe way
	my $datachannel = {
		fbamodel => $args->{fbamodel}
	};
	$args->{fbamodel}->_reference("NULL/".$args->{fbamodel}->id());
	if (!defined($args->{fbamodel}->getObject("biomasses","bio2"))) {
		$args->{fbamodel}->add("biomasses",Bio::KBase::constants::atp_hydrolysis_biomass());
	}
	Bio::KBase::ObjectAPI::functions::func_run_flux_balance_analysis({
		workspace => "NULL",
		fbamodel_id => $args->{fbamodel}->id(),
		fba_output_id => $args->{fbamodel}->id().".atp.fba",
		media_id => "RefGlucoseMinimal",
		media_workspace => Bio::KBase::utilities::conf("ModelSEED","default_media_workspace"),
		target_reaction => "bio2",
		reaction_addition_study => 1,
		max_objective_limit => $args->{max_objective_limit},
		reaction_list => [keys(%{$noncorehash})]
	},$datachannel);
	my $rxn_addition_data = $datachannel->{fba}->outputfiles()->{ReactionAdditionAnalysis};
	my $first = 1;
	if (defined($rxn_addition_data)) {
		for (my $i=1; $i < @{$rxn_addition_data}; $i++) {
			...
			if ($row->[2] == 0) {
				if ($row->[1] =~ m/(.)(rxn.+)/) {
					 ...
					 my $id = $2;
					 $first = 0;
					 my $rxnobj = $args->{fbamodel}->queryObject("modelreactions",{id => $id});
					 if ($sign eq "+") {
					 	if ($rxnobj->direction() eq "=") {
					 		$rxnobj->direction("<");
					 	} else {
					 		$args->{fbamodel}->remove("modelreactions",$rxnobj);
					 	}
					 } else {
					 	if ($rxnobj->direction() eq "=") {
					 		$rxnobj->direction(">");
