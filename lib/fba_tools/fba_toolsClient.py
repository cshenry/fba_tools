...
class fba_tools(object):
    ...
    def build_metabolic_model(self, params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method build_metabolic_model: argument json_rpc_context is not type dict as required.')
        resp = self._call('fba_tools.build_metabolic_model',
                          [params], json_rpc_context)
        return resp[0]
  
    def gapfill_metabolic_model(self, params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method gapfill_metabolic_model: argument json_rpc_context is not type dict as required.')
        resp = self._call('fba_tools.gapfill_metabolic_model',
                          [params], json_rpc_context)
        return resp[0]
  
    def run_flux_balance_analysis(self, params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method run_flux_balance_analysis: argument json_rpc_context is not type dict as required.')
        resp = self._call('fba_tools.run_flux_balance_analysis',
                          [params], json_rpc_context)
        return resp[0]
  
    def compare_fba_solutions(self, params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method compare_fba_solutions: argument json_rpc_context is not type dict as required.')
        resp = self._call('fba_tools.compare_fba_solutions',
                          [params], json_rpc_context)
        return resp[0]
  
    def propagate_model_to_new_genome(self, params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method propagate_model_to_new_genome: argument json_rpc_context is not type dict as required.')
        resp = self._call('fba_tools.propagate_model_to_new_genome',
                          [params], json_rpc_context)
        return resp[0]
  
    def simulate_growth_on_phenotype_data(self, params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method simulate_growth_on_phenotype_data: argument json_rpc_context is not type dict as required.')
        resp = self._call('fba_tools.simulate_growth_on_phenotype_data',
                          [params], json_rpc_context)
        return resp[0]
  
    def merge_metabolic_models_into_community_model(self, params, json_rpc_context = None):
        if json_rpc_context and type(json_rpc_context) is not dict:
            raise ValueError('Method merge_metabolic_models_into_community_model: argument json_rpc_context is not type dict as required.')
